 ...
 *   RANDDEV.C                                  *
 *                                              *
 *   Questa e' una libreria per la generazione  *
 *   di numeri pseudocasuali con varie distri-  *
 *   buzioni (RANDom DEViations).               *
 *                                              *
...
float Random (void)  { return rand2(&rand49_idum); }
...
{
	 //rand49_idum = -999999;  // 先完全重置（任何负数都可以）
     //SetRandomSeed((unsigned)time(NULL)); // Use the current time as the random seed
     //SetRandomSeed(12345);
    ...
    if (!initialized) 
	{
        SetRandomSeed(12345);  // 使用与main中相同的固定种子
        initialized = 1;
    }
...
{
   /*** Dichiarazione delle variabili locali. ***/
   float Dum;      /* Variabile di appoggio. */
...

{
   ...
   if (Set == 0)
     {do
        {v1 = (float)(2.0 * Random() - 1.0);
         v2 = (float)(2.0 * Random() - 1.0);
         r  = v1 * v1 + v2 * v2;}
      while (r >= 1.0 || r==0.0);
